#!/usr/bin/env bash
# Pipeline A：modular KV，独立进程 MODE=pruned | full
# 输出：A/output/${MODE}/
set -euo pipefail
...
if [[ "${MODE}" != "pruned" && "${MODE}" != "full" ]]; then
  echo "Pipeline A MODE must be pruned or full (got ${MODE})" >&2
  exit 1
fi
...
echo "=== Pipeline A modular MODE=${MODE} ==="
echo "LIMIT=${LIMIT} NUM_GPUS=${NUM_GPUS} backend=${PREDICTOR_BACKEND} pred=${PREDICTOR_DEVICE} prefetch=${PREFETCH_DEPTH} max_new=${LLM_MAX_NEW_TOKENS} cache=${PROMPT_CACHE_ROOT} out=${OUTPUT}"

if [[ "${CLEAR_CACHE}" == "1" ]]; then
  bash "${PIPELINE_ROOT}/common/clear_runtime_cache.sh" "A_${MODE}_before"
fi
...
if [[ "${CLEAR_CACHE}" == "1" ]]; then
  bash "${PIPELINE_ROOT}/common/clear_runtime_cache.sh" "A_${MODE}_after"
fi
